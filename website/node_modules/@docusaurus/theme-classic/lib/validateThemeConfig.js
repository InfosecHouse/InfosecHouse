...
const DocsSchema = _utilsValidation.Joi.object({
  ...
  sidebar: _utilsValidation.Joi.object({
    hideable: _utilsValidation.Joi.bool().default(DEFAULT_DOCS_CONFIG.sidebar.hideable),
    autoCollapseCategories: _utilsValidation.Joi.bool().default(DEFAULT_DOCS_CONFIG.sidebar.autoCollapseCategories)
...
}).nand('html', 'label') // We allow any unknown attributes on the links (users may need additional
...
const HtmlNavbarItemSchema = _utilsValidation.Joi.object({
  className: _utilsValidation.Joi.string(),
  type: _utilsValidation.Joi.string().equal('html').required(),
  value: _utilsValidation.Joi.string().required()
...
const CustomNavbarItemSchema = _utilsValidation.Joi.object({
  type: _utilsValidation.Joi.string().regex(CustomNavbarItemRegexp).required()
}).unknown();

const itemWithType = type => {
  // Because equal(undefined) is not supported :/
  const typeSchema = // eslint-disable-next-line no-nested-ternary
  type instanceof RegExp ? _utilsValidation.Joi.string().required().regex(type) : type ? _utilsValidation.Joi.string().required().equal(type) : _utilsValidation.Joi.string().forbidden();
...
const ThemeConfigSchema = _utilsValidation.Joi.object({
  ...
  prism: _utilsValidation.Joi.object({
    ...
    magicComments: _utilsValidation.Joi.array().items(_utilsValidation.Joi.object({
      className: _utilsValidation.Joi.string().required(),
      line: _utilsValidation.Joi.string(),
      block: _utilsValidation.Joi.object({
        start: _utilsValidation.Joi.string().required(),
        end: _utilsValidation.Joi.string().required()
      })
    }).or('line', 'block')).default(DEFAULT_CONFIG.prism.magicComments)
  }).default(DEFAULT_CONFIG.prism).unknown(),
  hideableSidebar: _utilsValidation.Joi.forbidden().messages({
    'any.unknown': 'themeConfig.hideableSidebar has been moved to themeConfig.docs.sidebar.hideable.'
  }),
  autoCollapseSidebarCategories: _utilsValidation.Joi.forbidden().messages({
